...
public class Map2_Bx2 : MonoBehaviour {
	public GameObject[] Dkbx_map22;
	...
	void OnTriggerEnter2D(Collider2D other) {
		if (activated1 == false && other.name == "Player") {
			for (int i = 0; i < Dkbx_map22.Length; i++)
				Dkbx_map22 [i].SetActive (true);
			activated1 = true;
			gos.SendMessage ("Heal", 80);
		}

		if (activated2 == true && other.name == "Player") {
			GameObject.Find ("Mine@64x64_162 (1)").SetActive(false);
			activated2 = false;
